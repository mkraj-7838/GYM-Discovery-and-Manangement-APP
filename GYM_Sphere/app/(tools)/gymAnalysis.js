...
import { View, Text, StyleSheet, ScrollView, ActivityIndicator, TouchableOpacity, Dimensions, Image } from 'react-native';
import { BarChart, PieChart, LineChart } from 'react-native-chart-kit';
import AsyncStorage from '@react-native-async-storage/async-storage';
import { AntDesign, MaterialIcons, FontAwesome, Feather, Ionicons, MaterialCommunityIcons } from '@expo/vector-icons';
...
const AnalyticsDashboard = () => {
  ...
  const getBusinessRecommendations = () => {
    ...
    if (stats.planStats.basic > stats.planStats.premium + stats.planStats.vip) {
      recommendations.push({
        icon: 'star',
        color: '#FFD700',
        text: 'Upsell premium plans: Offer upgrades with personal training sessions',
        action: 'Create upgrade campaign'
      });
    }
    
    // Attendance recommendations
    const avgDailyAttendance = stats.attendanceStats.daily.reduce((sum, day) => sum + day.count, 0) / stats.attendanceStats.daily.length;
    if (avgDailyAttendance < membersData.length * 0.3) {
      recommendations.push({
        icon: 'calendar',
        color: '#4CAF50',
        text: 'Low attendance: Consider loyalty programs or challenges',
        action: 'Setup attendance challenge'
      });
    }
    
    // Maintenance recommendations
    if (stats.maintenanceCost > membersData.length * 500) {
      recommendations.push({
        icon: 'tools',
        color: '#FF5722',
        text: 'High maintenance costs: Review equipment maintenance schedule',
        action: 'Optimize maintenance'
      });
    }
    
    // Gender balance recommendation
    if (stats.genderStats.female < membersData.length * 0.3) {
      recommendations.push({
        icon: 'gender-female',
        color: '#E91E63',
        text: 'Low female membership: Consider women-only classes or promotions',
        action: 'Create women-focused program'
      });
    }
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          <MaterialCommunityIcons name="chart-areaspline" size={28} color="#4CAF50" />
        ...
        {['daily', 'weekly', 'monthly'].map((range) => (
          ...
          >
            <MaterialIcons 
              name={
                range === 'daily' ? 'today' : 
                range === 'weekly' ? 'date-range' : 'calendar-today'
              } 
              size={16} 
      ...
      <View style={styles.summaryRow}>
        <View style={[styles.summaryCard, { backgroundColor: '#2D2D2D' }]}>
          ...
          <Text style={styles.summaryTrend}>
            <AntDesign name="caretup" size={12} color="#4CAF50" /> 12% from last month
          </Text>
        </View>
        ...
        <View style={[styles.summaryCard, { backgroundColor: '#2D2D2D' }]}>
          ...
          <Text style={styles.summaryTrend}>
            <AntDesign name="caretup" size={12} color="#4CAF50" /> 8% from last month
          </Text>
        </View>
        ...
        <View style={[styles.summaryCard, { backgroundColor: '#2D2D2D' }]}>
          ...
          <Text style={styles.summaryTrend}>
            <AntDesign name="caretdown" size={12} color="#F44336" /> 5% from last month
          </Text>
        </View>
      </View>
      ...
      {recommendations.length > 0 && (
        <View style={styles.recommendationsContainer}>
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.sectionTitle}>Business Boost Recommendations</Text>
          </View>
          ...
          {recommendations.map((rec, index) => (
            <TouchableOpacity key={index} style={styles.recommendationCard}>
              <View style={[styles.recommendationIcon, { backgroundColor: rec.color }]}>
                <MaterialCommunityIcons name={rec.icon} size={18} color="white" />
              </View>
      ...
      >
        ...
        {expandedChart === 'batch' && (
          <PieChart
            data={[
              { name: 'Morning', population: stats.batchStats.morning, color: '#4CAF50', legendFontColor: '#FFFFFF', legendFontSize: 12 },
              { name: 'Evening', population: stats.batchStats.evening, color: '#2196F3', legendFontColor: '#FFFFFF', legendFontSize: 12 }
            ]}
            width={screenWidth - 40}
            height={200}
            chartConfig={{
        ...
        <View style={styles.chartInsight}>
          <Text style={styles.insightText}>
            {stats.batchStats.morning > stats.batchStats.evening ? 
              "Morning batch is more popular. Consider adding more morning slots." : 
              "Evening batch is more popular. Consider promoting morning sessions."}
          </Text>
        </View>
      </TouchableOpacity>
      ...
      >
        <View style={styles.chartHeader}>
          <View style={styles.chartTitleContainer}>
            <MaterialCommunityIcons name="account-details" size={20} color="#2196F3" />
        ...
        {expandedChart === 'status' && (
          <BarChart
            data={{
              ...
              datasets: [
                {
                  data: [stats.statusStats.active, stats.statusStats.inactive, stats.statusStats.trial]
                }
              ]
            }}
            width={screenWidth - 40}
      ...
      >
        <View style={styles.chartHeader}>
          <View style={styles.chartTitleContainer}>
            <MaterialCommunityIcons name="gender-male-female" size={20} color="#E91E63" />
        ...
        {expandedChart === 'gender' && (
          <PieChart
            data={[
              { name: 'Male', population: stats.genderStats.male, color: '#2196F3', legendFontColor: '#FFFFFF', legendFontSize: 12 },
              { name: 'Female', population: stats.genderStats.female, color: '#E91E63', legendFontColor: '#FFFFFF', legendFontSize: 12 },
              { name: 'Other', population: stats.genderStats.other, color: '#9C27B0', legendFontColor: '#FFFFFF', legendFontSize: 12 }
            ]}
            width={screenWidth - 40}
            height={200}
            chartConfig={{
      ...
      >
        ...
        {expandedChart === 'plans' && (
          <BarChart
            data={{
              ...
              datasets: [
                {
                  data: [stats.planStats.basic, stats.planStats.premium, stats.planStats.vip]
                }
              ]
            }}
            width={screenWidth - 40}
        ...
        <View style={styles.chartInsight}>
          <Text style={styles.insightText}>
            {stats.planStats.basic > stats.planStats.premium + stats.planStats.vip ? 
              "Most members are on basic plans. Create premium plan incentives." : 
              "Premium plans are popular. Consider introducing VIP benefits."}
          </Text>
        </View>
      </TouchableOpacity>
      ...
      >
        <View style={styles.chartHeader}>
          ...
          <Feather 
            name={expandedChart === 'subscription' ? 'chevron-up' : 'chevron-down'} 
            size={20} 
            color="#888" 
          />
        </View>
        ...
        <View style={styles.chartInsight}>
          <Text style={styles.insightText}>
            {stats.subscriptionMonths.filter(m => m >= 6).length / membersData.length > 0.5 ? 
      ...
      >
        <View style={styles.chartHeader}>
          <View style={styles.chartTitleContainer}>
            <MaterialCommunityIcons name="chart-line" size={20} color="#00BCD4" />
            <Text style={styles.chartTitle}>Attendance Trend ({timeRange})</Text>
          </View>
          <Feather 
            name={expandedChart === 'attendance' ? 'chevron-up' : 'chevron-down'} 
            size={20} 
            color="#888" 
          />
        </View>
          ...
          <LineChart
            data={{
              labels: stats.attendanceStats[timeRange].map(item => 
                timeRange === 'daily' ? moment(item.date).format('D MMM') :
                timeRange === 'weekly' ? `Week ${moment(item.week).format('D MMM')}` :
                moment(item.month).format('MMM YYYY')
              ),
              datasets: [
                {
                  data: stats.attendanceStats[timeRange].map(item => item.count),
        ...
        <View style={styles.chartInsight}>
          <Text style={styles.insightText}>
            {timeRange === 'daily' ? "Peak days: " + 
              stats.attendanceStats.daily.slice().sort((a,b) => b.count - a.count).slice(0,2).map(d => moment(d.date).format('ddd')).join(', ') :
             timeRange === 'weekly' ? "Busiest weeks show " + 
              Math.max(...stats.attendanceStats.weekly.map(w => w.count)) + " visits" :
             "Monthly average: " + 
              Math.round(stats.attendanceStats.monthly.reduce((sum,m) => sum + m.count, 0) / stats.attendanceStats.monthly.length) + " visits/month"}
          </Text>
        </View>
      </TouchableOpacity>
      ...
      >
        <View style={styles.chartHeader}>
          ...
          <Feather 
            name={expandedChart === 'maintenance' ? 'chevron-up' : 'chevron-down'} 
            size={20} 
            color="#888" 
          />
        </View>
        ...
        {expandedChart === 'maintenance' && (
          <LineChart
            data={{
              labels: maintenanceData.map(item => moment(item.createdAt).format('MMM YYYY')),
              datasets: [
                {
